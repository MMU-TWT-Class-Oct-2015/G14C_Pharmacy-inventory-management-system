  ...
  && in_array($extension, $allowedExts) && $code != "" && $name != "" && $number != "" && $price != "" && $info != "") {
    if ($_FILES["file"]["error"] > 0) {
      ...
      move_uploaded_file($_FILES["file"]["tmp_name"],
        ...
        echo "Stored in: " . $path;
  	mysql_query("insert into product (ProductCode,ProductName,ProductQuantity,ProductPrice,ProductDescription,ProductImage,AdminID) values ('$code','$name',$number,'$price','$info','$path',$sess_aid)");
      }
    }
  else {
    echo "Invalid file";
